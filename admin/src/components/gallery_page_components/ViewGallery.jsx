      ...
      <div className="flex flex-col p-10 gap-10 overflow-y-scroll w-full">
        ...
        <div className="grid grid-cols-3 gap-3 phone:grid-cols-1">
          {images.results.map((image, index) => {
            const image_url = `${url}/images/${image.image}`;
            const author_img = `${url}/profile-images/${image.author.personal_info.profile_img}`;
            ...
            return (
              <div
                ...
                className="relative rounded-md h-[200px]"
              >
                <div
                  className={`
                ...
                >
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <p className="text-xs text-gray-100">{`Uploaded by ${author}`}</p>
                    </div>
                  </div>
                </div>
                <img
      ...
      {isOpen && (
        <div className="flex flex-col h-full w-full absolute top-0 left-0   bg-black bg-opacity-75  z-50  text-white">
          <div className="absolute top-0 left-0 flex w-full h-full">
            <img
              className="max-w-full max-h-full flex-shrink object-contain bg-red-400 mx-auto "
              src={`${url}/images/${images.results[currentIndex].image}`}
            />
          </div>

          <div className="flex flex-col justify-between h-full z-20">
            <div className="flex w-full items-center justify-between px-[3vw] py-5">
              <p>{images.results[currentIndex].desc}</p>
            ...
            <div className="flex w-full items-center justify-between px-[3vw] py-5">
              <div className="flex items-center gap-2">
                <img
                  src={`${url}/profile-images/${images.results[currentIndex].author.personal_info.profile_img}`}
                  alt=""
                  className="w-[20px] h-[20px] rounded-full bg-gray-50"
                />
                <p>{`Uploaded by ${images.results[currentIndex].desc}`}</p>
              </div>
            </div>
          </div>
