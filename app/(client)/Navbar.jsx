      ...
      <div className="tab-s:hidden flex gap-7">
        <ul className="flex items-center min-h-full ">
          {navlinks.map((link, index) => {
            let setMenuIsOpen;
            let menuIsOpen;
            if (link.path === "/about-us") {
              menuIsOpen = aboutMenuIsOpen;
              setMenuIsOpen = setAboutMenuIsOpen;
            }
            if (link.path === "/services") {
              menuIsOpen = serviceMenuOpen;
              setMenuIsOpen = setServiceMenuOpen;
            }
            return (
              <Link
                key={index}
                onMouseEnter={
                  link.path === "/about-us" || link.path === "/services"
                    ? () => setMenuIsOpen(false)
                    : () => {}
                }
                onMouseLeave={
                ...
                >
                  ...
                  >
                    ...
                    {(link.path === "/about-us" ||
                      link.path === "/services") && (
                      <i
                        className={` fi fi-br-angle-down  text-[8px] ${
                          !menuIsOpen ? "-rotate-180 mt-0 " : "rotate-0 mt-1"
                        } duration-75`}
                      ></i>
                    )}
                  </div>
                  ...
                  {link.path === "/about-us" ? (
                    <div
                      className={`${
                        menuIsOpen ? "h-0 p-0 my-0 hidden" : "h-fit border"
                      } absolute ${
                  ...
                  {link.path === "/services" ? (
                    ...
                    >
                      <div className="flex flex-col  gap-1 py-1">
                        ...
                        <Link
                          onClick={() => setMenuIsOpen(true)}
                          href="/services/hard-commodities"
                          className="hover:bg-gray-100 p-3 duration-75"
                        >
                          Hard Commodities
                        </Link>
              ...
              <div key={index}>
                <div className="flex justify-between ">
                  <Link
                    ...
                    className="text-[20px] py-3 w-full h-full"
                  >
                    {link.name}
                  </Link>
                  {(link.path === "/about-us" || link.path === "/services") && (
                    <div
                      ...
                      className="mr-2 hover:bg-gray-50 px-3 rounded-md flex items-center justify-center"
                    >
                      <i
                ...
                {link.path === "/services" ? (
                  <div
                    className={`${
                      menuIsOpen ? "gap-0 h-0 p-0 my-0" : "gap-1 h-fit my-1"
                    } flex flex-col w-full text-[18px] overflow-hidden duration-75`}
                  >
                    <Link
                      onClick={() => {
