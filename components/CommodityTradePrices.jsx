"use client";
import { getCommoditiesApi } from "@app/actions";
...
const CommodityTradePrices = () => {
  ...
  const fetchData = async () => {
    const data = await getCommoditiesApi();
    setData(data);
  };
        ...
        >
          ...
          >
            {data.map((commodity, index) => (
              <li
                key={index}
                className={`flex items-center gap-2 ${linkStyles} cursor-text`}
              >
                <p className="whitespace-nowrap uppercase">{commodity.name}</p>
                <p className="">${commodity.price}</p>
                <p
                  className={`${
                    !commodity.d_perc_change.startsWith("-")
                      ? "text-primary"
                      : "text-red-500"
                  } flex items-center`}
                >
                  ...
                  {!commodity.d_perc_change.startsWith("-")
                    ? commodity.d_perc_change
                    : commodity.d_perc_change.split("-")[1]}
                </p>
              </li>
            ))}
            {data.map((commodity, index) => (
              ...
              >
                <p className="whitespace-nowrap uppercase">{commodity.name}</p>
                <p>${Number(commodity.price).toLocaleString()}</p>
                <p
                  className={`${
                    !commodity.d_perc_change.startsWith("-")
                      ? "text-primary"
