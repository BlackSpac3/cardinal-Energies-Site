"use client";
import { setCookie, getCookie, encodeCookie } from "@utils";
...
const NewsLetterPopUp = () => {
  const newsLetterCookie = "__client_nlres";
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      const cookieData = encodeCookie("subsribed");

      setTimeout(() => {
        setCookie(newsLetterCookie, cookieData, 7);
  ...
  const close = () => {
    document.getElementById("newsletter-popup").close();
    const cookieData = encodeCookie("canceled");
    setCookie(newsLetterCookie, cookieData, 7);
  };

  useEffect(() => {
    const nlRes = getCookie(newsLetterCookie);
